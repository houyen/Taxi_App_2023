...
class FlutterwaveController extends Controller
{
    ...
    public function flutterwave_payment(Request $request)
	{
        ...
        $current_site_url = "https://" . $_SERVER['HTTP_HOST']; // Get the base URL of the current website

        $redirect_url = $current_site_url . "/api/flutterwave_callback?email=$email&amount=$amount_pay"; // Construct the full URL
        ...
        if (!$transaction->data || !property_exists($transaction->data, 'link')) {
       // There was an error from the API or the link is missing
       if ($transaction->message) {
        print_r('API returned error: ' . $transaction->message);
        } else {
        print_r('API returned error: Missing link in transaction data.');
     }
      } else {
          // Redirect to the page so the User can pay
